...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the Phase A+B+C histogram GBT primary model.")
...
def main() -> int:
    ...
    output_dir = args.output_dir / f"{args.league.lower()}_phase_abc_gbt"
    ...
    model = build_hist_gbt_pipeline(feature_cols)
...
def _log_mlflow_run(
    ...
    run_name = args.run_name or f"phase-abc-gbt-{args.league.lower()}"
    tags = {
        "hypothesis": (
            "Tree-based Phase A+B+C features improve tournament Brier over linear baselines"
        ),
        ...
        "retest_if": "phase A/B/C definitions or tree hyperparameters change",
