...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the women's CloseWR3 challenger.")
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument("--run-name", default="feat-15-women-closewr3")
...
def main() -> int:
    ...
    feature_cols = [
        "seed_diff",
        "elo_diff",
        "women_hca_adj_qg_diff",
        "close_win_pct_diff",
    ]

    output_dir = args.output_dir / "w_women_closewr3"
    output_dir.mkdir(parents=True, exist_ok=True)
    if args.save_features:
        feature_table.to_parquet(output_dir / "women_closewr3_features.parquet", index=False)
...
def _log_mlflow_run(
    ...
    tags = {
        "hypothesis": (
            "Women's CloseWR3 may add signal beyond seed, Elo, and the HCA-adjusted quality gap"
        ),
        "model_family": "logistic_regression",
        "league": "women",
        "season_window": f"pre-{min(args.holdout_seasons)}",
        "depends_on": (
            "feature:elo_v1,feature:feat_13_women_hca_adj_qg_v1,feature:feat_15_women_closewr3_v1"
        ),
        "retest_if": "close-game threshold or women HCA definition changes",
        ...
        mlflow.log_params(
            {
                ...
                "features": ("seed_diff,elo_diff,women_hca_adj_qg_diff,close_win_pct_diff"),
