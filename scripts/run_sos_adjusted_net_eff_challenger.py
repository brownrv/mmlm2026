...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the SOS-adjusted net-efficiency challenger.")
...
def main() -> int:
    ...
    feature_cols = ["seed_diff", "elo_diff", "sos_adjusted_net_eff_diff"]

    output_dir = args.output_dir / f"{args.league.lower()}_sos_adj_net_eff"
    output_dir.mkdir(parents=True, exist_ok=True)
    if args.save_features:
        feature_table.to_parquet(output_dir / "sos_adj_net_eff_features.parquet", index=False)
...
def _log_mlflow_run(
    ...
    run_name = args.run_name or f"feat-11-sos-adj-net-eff-{args.league.lower()}"
    tags = {
        "hypothesis": (
            "SOS-adjusted net efficiency plus Elo may improve held-out "
            "tournament Brier over the current raw baselines"
        ),
        "model_family": "logistic_regression",
        "league": "men" if args.league == "M" else "women",
        "season_window": f"pre-{min(args.holdout_seasons)}",
        "depends_on": "feature:elo_v1,feature:phase_b_v1,feature:feat_11_sos_adj_net_eff_v1",
        "retest_if": "SOS-adjusted net-efficiency definition changes",
        ...
        mlflow.log_params(
            {
                ...
                "features": "seed_diff,elo_diff,sos_adjusted_net_eff_diff",
