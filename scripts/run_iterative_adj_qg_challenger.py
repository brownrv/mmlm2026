...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the iterative adjusted-quality-gap challenger."
    )
...
def main() -> int:
    ...
    feature_cols = ["seed_diff", "elo_diff", "adj_qg_diff"]

    output_dir = args.output_dir / f"{args.league.lower()}_iter_adj_qg"
    output_dir.mkdir(parents=True, exist_ok=True)
    if args.save_features:
        feature_table.to_parquet(output_dir / "iter_adj_qg_features.parquet", index=False)
...
def _log_mlflow_run(
    ...
    run_name = args.run_name or f"feat-12-iter-adj-qg-{args.league.lower()}"
    tags = {
        "hypothesis": (
            "Iterative adjusted efficiency plus Elo may improve held-out "
        ...
        "depends_on": "feature:elo_v1,feature:feat_12_iter_adj_qg_v1",
        "retest_if": "iterative adjusted-efficiency definition changes",
        ...
        mlflow.log_params(
            {
                ...
                "features": "seed_diff,elo_diff,adj_qg_diff",
