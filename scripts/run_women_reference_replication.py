...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the fuller women reference-style replication challenger."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument("--run-name", default="val-05-women-reference-replication")
    ...
    parser.add_argument("--elo-alpha", type=float, default=0.10)
...
def main() -> int:
    ...
    output_dir = args.output_dir / "w_reference_replication"
    output_dir.mkdir(parents=True, exist_ok=True)
    if args.save_features:
        feature_table.to_parquet(output_dir / "women_reference_features.parquet", index=False)
        team_features.to_parquet(output_dir / "phase_ab_team_features.parquet", index=False)

    summary = _validate_reference_holdouts(
        feature_table,
        feature_cols=feature_cols,
        holdout_seasons=args.holdout_seasons,
        elo_scale=args.elo_scale,
        elo_alpha=args.elo_alpha,
    )
    ...
    infer_frame["Pred"] = ((1.0 - args.elo_alpha) * base_pred + args.elo_alpha * elo_pred).clip(
        lower=0.025,
        upper=0.975,
    )
    infer_frame["ID"] = [
...
def _validate_reference_holdouts(
    frame: pd.DataFrame,
    *,
    ...
    for holdout_season in holdout_seasons:
        ...
        pred = ((1.0 - elo_alpha) * base_pred + elo_alpha * elo_pred).clip(
            lower=0.025,
            upper=0.975,
        )
...
def _log_mlflow_run(
    ...
    tags = {
        "hypothesis": (
            "A fuller women reference-style replication with tuned carryover Elo "
            "may beat the current frozen women baseline"
        ),
        "model_family": "logistic_regression_plus_fixed_elo_blend",
        "league": "women",
        "season_window": f"pre-{min(args.holdout_seasons)}",
        "depends_on": (
            "feature:feat_13_women_hca_adj_qg_v1,feature:feat_15_women_closewr3_v1,"
            "feature:elo_tuned_carryover_women_v1"
        ),
        "retest_if": "women reference replication parameters or feature set changes",
