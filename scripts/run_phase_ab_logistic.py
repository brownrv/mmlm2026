...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the Phase A+B logistic primary model.")
...
def main() -> int:
    ...
    feature_cols = phase_ab_feature_columns(args.league)

    output_dir = args.output_dir / f"{args.league.lower()}_phase_ab"
    output_dir.mkdir(parents=True, exist_ok=True)
    if args.save_features:
        feature_table.to_parquet(output_dir / "phase_ab_tourney_features.parquet", index=False)
...
def _log_mlflow_run(
    ...
    run_name = args.run_name or f"phase-ab-logistic-{args.league.lower()}"
    tags = {
        "hypothesis": "Phase A+B features improve tournament Brier over seed and Elo baselines",
        "model_family": "logistic_regression",
        "league": "men" if args.league == "M" else "women",
        "season_window": f"pre-{min(args.holdout_seasons)}",
        "depends_on": "feature:seed_diff_v1,feature:elo_v1,feature:phase_b_v1",
        "retest_if": "phase A/B definitions or regular season cutoffs change",
