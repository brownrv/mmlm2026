...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Tune women Elo parameters with persistent Optuna TPE."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
...
def main() -> int:
    ...
    regular = pd.read_csv(args.data_dir / "WRegularSeasonCompactResults.csv")
    tourney = pd.read_csv(args.data_dir / "WNCAATourneyCompactResults.csv")
    ...
    narrow_best = run_study(
        study_name=f"women_{args.mode}_narrow",
        storage_path=storage_path,
        games=games,
        objective_seasons=objective_seasons,
        seed=args.seed,
